...
must_haves:
  truths:
    ...
    - "Shaped reward metric constants are defined in metric_schema.py"
  artifacts:
...
<task type="auto">
  ...
  <action>
    ...
    **Step 3: Call _update_wandb_config() in get() on first call.**
    ...
    In the `get()` method, add `self._update_wandb_config()` immediately after the `init_debate_workspace()` call inside the `if self._get_count == 1:` block. This ensures:
    ...
    **Step 4: Replace hardcoded metric strings in _compute_and_apply_shaped_rewards().**

    In `_compute_and_apply_shaped_rewards()`, import and use the new metric constants instead of hardcoded strings:
    - Replace `"debate/shaped_reward/mean"` with `METRIC_SHAPED_REWARD_MEAN`
    - Replace `f"debate/shaped_reward/{role}"` with the corresponding `METRIC_SHAPED_REWARD_SOLVER`, `METRIC_SHAPED_REWARD_VERIFIER`, `METRIC_SHAPED_REWARD_JUDGE` using a lookup dict `{"solver": METRIC_SHAPED_REWARD_SOLVER, "verifier": METRIC_SHAPED_REWARD_VERIFIER, "judge": METRIC_SHAPED_REWARD_JUDGE}`
    - Replace `"debate/shaped_reward/strategy_active"` with `METRIC_SHAPED_REWARD_STRATEGY_ACTIVE`
