"""SWEEP config for identity (baseline) reward shaping (SmolLM-135M, MATH-500).

Identity passthrough: no reward shaping applied. Raw debate rewards used directly.
This serves as the control condition for the reward shaping comparison experiment.

Usage:
    # Preview (no submission)
    uv run python configs/reward_shaping_sweep/sweep_identity.py start

    # Submit to cluster
    uv run python configs/reward_shaping_sweep/sweep_identity.py --submit start
