"""Qwen3-4B CooperBench: High learning rate (5e-6).

Same as main DAPO config but with 5x higher peak LR (5e-6 vs 1e-6).
Tests whether more aggressive learning speeds up reward improvement.
"""
...
_PEAK_LR = 5e-6  # 5x higher than default 1e-6


class Qwen3_4bCooperBenchLrHigh(sweep_base.Sweep):
    ...
    fax: sweep.FaxConfig = sweep_base.PostTraining(
        ...
        patch_run_config=dict(
            ...
            lr_schedule={"kwargs": {"warmup_steps": 20, "total_steps": _TOTAL_TRAIN_STEPS, "peak_lr": _PEAK_LR, "end_lr": 5e-7}},
            seed=SEED,
...
if __name__ == "__main__":
    sweep.cli.run_sweep_with_flags(Qwen3_4bCooperBenchLrHigh(), debugging_artefacts=[__file__])
