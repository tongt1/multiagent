"""Qwen3-4B CooperBench 6GPU (Condition B): 4 train + 2 vLLM GPUs.

GPU allocation experiment variant. Identical to baseline except:
- num_training_gpus=4, num_sampling_gpus=2 (6 total vs 16 baseline)
- n_tensor_parallel=4 in run config
...
Usage:
    ...
    PYTHONPATH=/mnt/data/terry/repos/post_training/src:/mnt/data/terry/home/reward-training \
        ...
        configs/reward_shaping_sweep/sweep_qwen3_4b_cooperbench_6gpu.py start

    # Submit to cluster
    PYTHONPATH=/mnt/data/terry/repos/post_training/src:/mnt/data/terry/home/reward-training \
        /mnt/data/terry/repos/post_training/.venv/bin/python \
        configs/reward_shaping_sweep/sweep_qwen3_4b_cooperbench_6gpu.py --submit start
"""
...
# Model profile for Qwen3-4B (6 GPU condition: 4 train + 2 vLLM)
...
# Paths
RUN_CONFIG_PATH = "${HOME}/reward-training/run_configs/qwen3_4b_cooperbench_6gpu.run"
...
class Qwen3_4bCooperBench6Gpu(sweep_base.Sweep):
    settings: sweep.SweepSettings = sweep.SweepSettings(
        sweep_output_path="${HOME}/sweep_jobs/qwen3_4b_cooperbench_6gpu/",
...
if __name__ == "__main__":
    sweep.cli.run_sweep_with_flags(Qwen3_4bCooperBench6Gpu(), debugging_artefacts=[__file__])
