"""Qwen3-4B CooperBench DUAL-MODEL: train-both with advantage alignment.

Both solver and verifier receive gradients. Advantage alignment creates
cooperative learning signal via cross-agent advantage correlation (Duque Van et al.).
"""
...
class Qwen3_4bCooperBenchDualAA(sweep_base.Sweep):
        ...
        patch_run_config=dict(
            ...
            minizord=flink_zord.FlinkZordConfig(
                ...
                actor=FlinkCombActorConfig(
                    ...
                    agentic_temperature=1.0, agentic_comms_mode=True,
                ),
                num_actors_per_batch_item=4,
                actors_queue_batches=32,
                eval_actors_queue_batches=32,
                # KEY: Dual-model learner, train both, with advantage alignment
                learner=FlinkDualRlooLearnerConfig(
                    policy_gradient_loss="grpo",
                    solver_ckpt=CKPT_PATH,
                    verifier_ckpt=CKPT_PATH,
                    freeze_roles=[],  # train both models
...
if __name__ == "__main__":
    sweep.cli.run_sweep_with_flags(Qwen3_4bCooperBenchDualAA(), debugging_artefacts=[__file__])
