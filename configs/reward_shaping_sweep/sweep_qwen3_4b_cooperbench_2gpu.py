"""Qwen3-4B CooperBench 2GPU (Condition D): 1 train + 1 vLLM GPU.

GPU allocation experiment variant. Identical to baseline except:
- num_training_gpus=1, num_sampling_gpus=1 (2 total vs 16 baseline)
- n_tensor_parallel=1 in run config
...
Usage:
    ...
    PYTHONPATH=/mnt/data/terry/repos/post_training/src:/mnt/data/terry/home/reward-training \
        ...
        configs/reward_shaping_sweep/sweep_qwen3_4b_cooperbench_2gpu.py start

    # Submit to cluster
    PYTHONPATH=/mnt/data/terry/repos/post_training/src:/mnt/data/terry/home/reward-training \
        /mnt/data/terry/repos/post_training/.venv/bin/python \
        configs/reward_shaping_sweep/sweep_qwen3_4b_cooperbench_2gpu.py --submit start
"""
...
from configs.model_profiles import QWEN3_4B_INSTRUCT_2GPU

# Model profile for Qwen3-4B (2 GPU condition: 1 train + 1 vLLM)
_PROFILE = QWEN3_4B_INSTRUCT_2GPU
...
# Paths
RUN_CONFIG_PATH = "${HOME}/reward-training/run_configs/qwen3_4b_cooperbench_2gpu.run"
...
class Qwen3_4bCooperBench2Gpu(sweep_base.Sweep):
    settings: sweep.SweepSettings = sweep.SweepSettings(
        sweep_output_path="${HOME}/sweep_jobs/qwen3_4b_cooperbench_2gpu/",
    ...
    fax: sweep.FaxConfig = sweep_base.PostTraining(
        ...
        ckpt_path=CKPT_PATH,
        output_dir="s3://us-east-01a/30d/post-training/${USER}/multiagent-debate-rl/qwen3-4b-cooperbench-2gpu/${SWEEP_NAME}/${TRIAL_IDX}",
...
if __name__ == "__main__":
    sweep.cli.run_sweep_with_flags(Qwen3_4bCooperBench2Gpu(), debugging_artefacts=[__file__])
