"""SWEEP config for reward mixing shaping (SmolLM-135M, MATH-500).

Reward mixing: r_shaped = alpha * G + (1 - alpha) * local_reward.
Blends the global team reward with local per-agent reward using alpha=0.5.

Usage:
    # Preview (no submission)
    uv run python configs/reward_shaping_sweep/sweep_reward_mixing.py start

    # Submit to cluster
    uv run python configs/reward_shaping_sweep/sweep_reward_mixing.py --submit start
