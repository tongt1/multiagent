"""Qwen3-4B CooperBench: Reward credit assignment strategies.

Dual-model (freeze verifier) with different reward shaping strategies:
- difference_rewards: D_i = G(z) - G(z_{-i}) marginal contribution
- coma_advantage: COMA counterfactual baseline
- potential_based: r' = r + gamma*Phi(s')-Phi(s)
- reward_mixing: r' = alpha*G + (1-alpha)*local

Each strategy transforms raw Docker test rewards before GRPO loss.
Uses 4 gens/prompt for meaningful group-level shaping.
"""
...
_EVAL_BATCH_SIZE = 4
_GENERATIONS_PER_PROMPT = 4  # Need multiple gens for credit assignment
_MAX_TURNS = 20
_REDUNDANCY_FACTOR = 2.0

# Credit assignment strategy to sweep over (set via env or override)
_REWARD_STRATEGY = "difference_rewards"


def _make_sweep(strategy: str, strategy_params: dict | None = None):
    """Factory for building sweep configs with different reward shaping strategies."""
    return dict(
        ...
        objective={"loss": {"kwargs": {
            ...
            "preference": {
                ...
                "dapo_overlong_coeff": 0.5,
                "dapo_overlong_threshold": 0.9,
            },
        }}},
        ...
        minizord=flink_zord.FlinkZordConfig(
            ...
            learner=FlinkDualRlooLearnerConfig(
                ...
                solver_ckpt=CKPT_PATH, verifier_ckpt=CKPT_PATH,
                freeze_roles=["verifier", "judge"],
            ),
            actor_outputs_streamers=[DebateMetricStreamerConfig(
                n_rollouts_per_prompt=_GENERATIONS_PER_PROMPT,
                reward_shaping_strategy=strategy,
                reward_shaping_params=strategy_params or {},
            )],
...
class Qwen3_4bCooperBenchCredit(sweep_base.Sweep):
    ...
    fax: sweep.FaxConfig = sweep_base.PostTraining(
        ...
        jobs_max_fanout=4,  # 4 trials in parallel (one per strategy)
        ...
        patch_run_config=_make_sweep(_REWARD_STRATEGY),
    ...
    def get_search_space(self) -> Iterable[sweep.ParamOverrides]:
        # Each trial uses a different reward shaping strategy
        # NOTE: sweep framework doesn't easily override nested DebateMetricStreamerConfig.
        # Submit each strategy as a separate sweep instead.
        return [{}]


if __name__ == "__main__":
    sweep.cli.run_sweep_with_flags(Qwen3_4bCooperBenchCredit(), debugging_artefacts=[__file__])
