"""SWEEP config for potential-based reward shaping (SmolLM-135M, MATH-500).

Potential-based shaping (Ng et al.): F(s,s') = gamma * Phi(s') - Phi(s).
Uses debate_length as the potential function with gamma=0.99.
Preserves optimal policy under mild conditions.

Usage:
    # Preview (no submission)
    uv run python configs/reward_shaping_sweep/sweep_potential_based.py start

    # Submit to cluster
    uv run python configs/reward_shaping_sweep/sweep_potential_based.py --submit start
