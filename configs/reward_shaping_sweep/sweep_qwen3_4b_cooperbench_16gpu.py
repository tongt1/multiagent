"""Qwen3-4B CooperBench 16GPU (Condition A): 8 train + 8 vLLM GPUs.

GPU allocation experiment baseline. Identical to main config except
50 training steps for throughput measurement.
...
Usage:
    ...
    PYTHONPATH=/mnt/data/terry/repos/post_training/src:/mnt/data/terry/home/reward-training \
        ...
        configs/reward_shaping_sweep/sweep_qwen3_4b_cooperbench_16gpu.py start

    # Submit to cluster
    PYTHONPATH=/mnt/data/terry/repos/post_training/src:/mnt/data/terry/home/reward-training \
        /mnt/data/terry/repos/post_training/.venv/bin/python \
        configs/reward_shaping_sweep/sweep_qwen3_4b_cooperbench_16gpu.py --submit start
"""
...
class Qwen3_4bCooperBench16Gpu(sweep_base.Sweep):
    settings: sweep.SweepSettings = sweep.SweepSettings(
        sweep_output_path="${HOME}/sweep_jobs/qwen3_4b_cooperbench_16gpu/",
    ...
    fax: sweep.FaxConfig = sweep_base.PostTraining(
        ...
        ckpt_path=CKPT_PATH,
        output_dir="s3://us-east-01a/30d/post-training/${USER}/multiagent-debate-rl/qwen3-4b-cooperbench-16gpu/${SWEEP_NAME}/${TRIAL_IDX}",
...
if __name__ == "__main__":
    sweep.cli.run_sweep_with_flags(Qwen3_4bCooperBench16Gpu(), debugging_artefacts=[__file__])
