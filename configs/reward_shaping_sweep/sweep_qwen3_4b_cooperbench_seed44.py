"""Qwen3-4B CooperBench Seed 44: Multi-seed training run for variance measurement.

One of 3 seed configs (42, 43, 44) for measuring training variance across seeds.
Derived from sweep_qwen3_4b_cooperbench.py (base config) with:
- SEED = 44
- force_at_init=True for both validation and checkpoint (step-0 baseline)
- Seed-disambiguated output paths

Step-0 eval metrics from validation(force_at_init=True) serve as the pre-training
baseline (no separate pre-training eval run needed). The step-0 checkpoint is also
saved for optional standalone re-evaluation.
...
Usage:
    PYTHONPATH=/mnt/data/terry/repos/post_training/src:/mnt/data/terry/home/reward-training \\
        ...
        configs/reward_shaping_sweep/sweep_qwen3_4b_cooperbench_seed44.py --submit start
"""
...
# Training params (identical to base config)
...
# DAPO / Loss configuration (identical to base config)
_LOSS_VARIATION = "dapo"
_TEMPERATURE_TRAIN = 1.0
_TEMPERATURE_EVAL = 0.6
_MAX_TURNS = 20
_DAPO_OVERLONG_COEFF = 0.5
_DAPO_OVERLONG_THRESHOLD = 0.9
_DAPO_MIN_VALID_PER_GROUP = 2
_DAPO_MAX_BATCH_MASK_RATIO = 0.5
_REDUNDANCY_FACTOR = 1.25


class Qwen3_4bCooperBenchSeed44(sweep_base.Sweep):
    settings: sweep.SweepSettings = sweep.SweepSettings(
        sweep_output_path="${HOME}/sweep_jobs/qwen3_4b_cooperbench_seed44/",
    ...
    fax: sweep.FaxConfig = sweep_base.PostTraining(
        ...
        ckpt_path=CKPT_PATH,
        output_dir="s3://us-east-01a/30d/post-training/${USER}/multiagent-debate-rl/qwen3-4b-cooperbench-seed44/${SWEEP_NAME}/${TRIAL_IDX}",
        patch_run_config=dict(
            ...
            # force_at_init=True: runs validation + saves checkpoint at step 0
            # Step-0 eval metrics = pre-training baseline for comparison
...
if __name__ == "__main__":
    sweep.cli.run_sweep_with_flags(Qwen3_4bCooperBenchSeed44(), debugging_artefacts=[__file__])
