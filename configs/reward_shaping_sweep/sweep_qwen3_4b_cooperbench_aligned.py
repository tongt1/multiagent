"""Qwen3-4B CooperBench DUAL-MODEL: advantage-aligned train-both mode with Docker-based verifiable rewards.

Based on sweep_qwen3_4b_cooperbench_dual.py but uses train-both mode (no frozen roles)
and enables advantage alignment between solver and verifier models. Both models
receive gradients on their role-specific tokens, and the advantage alignment
creates a cooperative learning signal via cross-agent advantage correlation.

Key differences from sweep_qwen3_4b_cooperbench_dual.py:
- freeze_roles=[] (train-both mode, both models receive gradients)
- advantage_alignment_enabled=True (beta=1.0, gamma=0.99)
- PYTHONPATH includes cooperbench-dual repo root for advantage alignment imports

Usage:
    # Preview (no submission)
    PYTHONPATH=/mnt/data/terry/repos/post_training/src:/mnt/data/terry/home/cooperbench-dual \
        /mnt/data/terry/repos/post_training/.venv/bin/python \
        configs/reward_shaping_sweep/sweep_qwen3_4b_cooperbench_aligned.py start

    # Submit to cluster
    PYTHONPATH=/mnt/data/terry/repos/post_training/src:/mnt/data/terry/home/cooperbench-dual \
        /mnt/data/terry/repos/post_training/.venv/bin/python \
        configs/reward_shaping_sweep/sweep_qwen3_4b_cooperbench_aligned.py --submit start
"""
...
FREEZE_ROLES: list[str] = []  # train-both mode: both models receive gradients
...
class Qwen3_4bCooperBenchAligned(sweep_base.Sweep):
    settings: sweep.SweepSettings = sweep.SweepSettings(
        sweep_output_path="${HOME}/sweep_jobs/qwen3_4b_cooperbench_aligned/",
    ...
    fax: sweep.FaxConfig = sweep_base.PostTraining(
        ...
        ckpt_path=CKPT_PATH,
        output_dir="s3://us-east-01a/30d/post-training/${USER}/multiagent-debate-rl/qwen3-4b-cooperbench-aligned/${SWEEP_NAME}/${TRIAL_IDX}",
        patch_run_config=dict(
            wandb=dict(
                project_name=WANDB_PROJECT,
                tags=["marti", "qwen3-4b", "dual", "cooperbench", "train_both", "advantage_alignment"],
            ),
            metadata=dict(
                multi_model=dict(
                    enabled=True,
            ...
            minizord=flink_zord.FlinkZordConfig(
                ...
                # KEY: FlinkDualRlooLearnerConfig with advantage alignment enabled
                learner=FlinkDualRlooLearnerConfig(
                    policy_gradient_loss="grpo",
                    solver_ckpt=SOLVER_CKPT_PATH,
                    verifier_ckpt=VERIFIER_CKPT_PATH,
                    freeze_roles=[],  # train-both mode
                    advantage_alignment_enabled=True,
                    advantage_alignment_beta=1.0,
                    advantage_alignment_gamma=0.99,
                ),
...
if __name__ == "__main__":
    sweep.cli.run_sweep_with_flags(Qwen3_4bCooperBenchAligned(), debugging_artefacts=[__file__])
