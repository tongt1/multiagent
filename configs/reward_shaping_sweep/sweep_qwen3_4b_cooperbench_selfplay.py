"""Qwen3-4B CooperBench: Self-play / parameter sharing.

Both agents are the same model — single model trains on all tokens from
both agent 1 and agent 2. No role masking, no dual learner.
Uses standard RLOO learner with comms mode rollouts.
"""
...
class Qwen3_4bSelfPlay(sweep_base.Sweep):
    ...
    fax: sweep.FaxConfig = sweep_base.PostTraining(
        ...
        patch_run_config=dict(
            ...
            minizord=flink_zord.FlinkZordConfig(
                ...
                # KEY: Single-model learner — both agents share weights, all tokens get gradients
...
if __name__ == "__main__":
    sweep.cli.run_sweep_with_flags(Qwen3_4bSelfPlay(), debugging_artefacts=[__file__])
