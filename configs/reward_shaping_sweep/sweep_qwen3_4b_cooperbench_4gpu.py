"""Qwen3-4B CooperBench 4GPU (Condition C): 2 train + 2 vLLM GPUs.

GPU allocation experiment variant. Identical to baseline except:
- num_training_gpus=2, num_sampling_gpus=2 (4 total vs 16 baseline)
- n_tensor_parallel=2 in run config
- 50 training steps (throughput measurement only)
...
Usage:
    ...
    PYTHONPATH=/mnt/data/terry/repos/post_training/src:/mnt/data/terry/home/reward-training \
        ...
        configs/reward_shaping_sweep/sweep_qwen3_4b_cooperbench_4gpu.py start

    # Submit to cluster
    PYTHONPATH=/mnt/data/terry/repos/post_training/src:/mnt/data/terry/home/reward-training \
        /mnt/data/terry/repos/post_training/.venv/bin/python \
        configs/reward_shaping_sweep/sweep_qwen3_4b_cooperbench_4gpu.py --submit start
"""
...
from configs.model_profiles import QWEN3_4B_INSTRUCT_4GPU

# Model profile for Qwen3-4B (4 GPU condition: 2 train + 2 vLLM)
_PROFILE = QWEN3_4B_INSTRUCT_4GPU
NUM_TRAINING_GPUS = _PROFILE.num_training_gpus      # 2
...
# Paths
RUN_CONFIG_PATH = "${HOME}/reward-training/run_configs/qwen3_4b_cooperbench_4gpu.run"
...
# Training params: 50 steps for GPU allocation throughput experiment
...
class Qwen3_4bCooperBench4Gpu(sweep_base.Sweep):
    settings: sweep.SweepSettings = sweep.SweepSettings(
        sweep_output_path="${HOME}/sweep_jobs/qwen3_4b_cooperbench_4gpu/",
    ...
    fax: sweep.FaxConfig = sweep_base.PostTraining(
        ...
        ckpt_path=CKPT_PATH,
        output_dir="s3://us-east-01a/30d/post-training/${USER}/multiagent-debate-rl/qwen3-4b-cooperbench-4gpu/${SWEEP_NAME}/${TRIAL_IDX}",
        patch_run_config=dict(
            ...
            minizord=flink_zord.FlinkZordConfig(
                ...
                # Pipeline depth: with 16 unique prompts per batch and 1 gen/prompt,
                # 32 queue batches = ~512 trajectories buffered ahead.
...
if __name__ == "__main__":
    sweep.cli.run_sweep_with_flags(Qwen3_4bCooperBench4Gpu(), debugging_artefacts=[__file__])
