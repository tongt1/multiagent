"""Qwen3-4B CooperBench: RLOO (CoPG with RLOO-token-KL variant).

Uses CoPG loss with RLOO_TOKEN_KL variant. Unlike GRPO/DAPO which use PPO-style
clipping, RLOO uses leave-one-out baseline estimation and token-level KL.
Requires generations_per_prompt >= 2 for pairwise comparison.

NOT using DAPO defaults (per user request: DAPO defaults for all EXCEPT RLOO).
"""
...
_TRAIN_BATCH_SIZE = 16  # 4 prompts * 4 gens = 16
_EVAL_BATCH_SIZE = 4
_GENERATIONS_PER_PROMPT = 4  # RLOO needs >= 2 for leave-one-out
_MAX_TURNS = 20
_REDUNDANCY_FACTOR = 2.0


class Qwen3_4bCooperBenchRloo(sweep_base.Sweep):
    ...
    fax: sweep.FaxConfig = sweep_base.PostTraining(
        ...
        patch_run_config=dict(
            ...
            objective={"loss": {"kwargs": {
                ...
                "preference": {
                    # RLOO: CoPG with token-level KL
                    "loss_variation": "rloo-token-kl",
                    "use_reference_policy": True,  # Required for RLOO KL
                    "beta": 0.03,  # Standard KL coefficient
            ...
            minizord=flink_zord.FlinkZordConfig(
                ...
                # KEY: CoPG learner (not GRPO)
                learner=FlinkRlooLearnerConfig(policy_gradient_loss="copg"),
...
if __name__ == "__main__":
    sweep.cli.run_sweep_with_flags(Qwen3_4bCooperBenchRloo(), debugging_artefacts=[__file__])
