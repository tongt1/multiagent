"""SWEEP config for difference rewards shaping (SmolLM-135M, MATH-500).

Difference rewards: D_i = G - G_{-i}. Each agent's reward is the marginal
contribution of that agent to the team reward.

Usage:
    # Preview (no submission)
    uv run python configs/reward_shaping_sweep/sweep_difference_rewards.py start

    # Submit to cluster
    uv run python configs/reward_shaping_sweep/sweep_difference_rewards.py --submit start
